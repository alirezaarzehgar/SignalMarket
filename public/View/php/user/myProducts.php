...
<?php while ($admin = $allAdmins->fetch_assoc()) : ?>
    ...
    <?php foreach ($PRepo->getProductsByAdminName($admin['username'] ?: "") as $value) : ?>
        <?php if (
            ...
            !isset($value['final_product_path'])
        ) continue; ?>
            ...
            <div id="overally-<?= $value['id'] ?>" class="overally <?php if (!isset($_SESSION['user'])) echo "hidden"; ?>">
                <div class="container p-3 h-100">
                    <form class="d-flex flex-column justify-content-around h-75" action="<?= $_SERVER['PHP_SELF'] ?>" method="POST">
                        <h3 class="text-white"> my product </h3>

                        <h6 class="text-center text-white border p-2">
                            <a href="/assets/finals/<?= md5(htmlentities($value['subject']) . $value['admin_name']) . $value['final_product_path'] ?>"><?= $value['final_product_path'] ?></a>
                        </h6>

                        <h6 class="text-center text-white border p-2">
                            <a class="text-white" href="/assets/sent_signal_dir_path/<?= md5($value['customer_name']) . $value['sent_signal_dir_path'] ?>"> <?= $value['sent_signal_dir_path'] ?> </a>
                        </h6>
